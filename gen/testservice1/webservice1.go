...
type OrderMsgSendJaxbResponse struct {
	XMLName	xml.Name	`xml:"orderMsgSendJaxbResponse"`
	Action	string		`xml:"-"`
...
type BatchOrderMsgSendJaxbResponse struct {
	XMLName	xml.Name	`xml:"batchOrderMsgSendJaxbResponse"`
	Action	string		`xml:"-"`
...
type BatchCustomMessageSendJaxbResponse struct {
	XMLName	xml.Name	`xml:"batchCustomMessageSendJaxbResponse"`
	Action	string		`xml:"-"`
...
type QueryModelResponse struct {
	XMLName	xml.Name	`xml:"queryModelResponse"`
	Action	string		`xml:"-"`
...
func (s *MessageServiceJaxbImplService) OrderMsgSendJaxb(p MessageReqJaxb) (r *MessageResJaxb, err error) {
	si := OrderMsgSendJaxb{}
	si.Action = "http://ws.service.creditease.com/MessageService2.0/orderMsgSendJaxbRequest"
	si.RequestList=p
...
func (s *MessageServiceJaxbImplService) BatchOrderMsgSendJaxb(p MessageReqJaxb) (r *MessageResJaxb, err error) {
	...
	si.Action = "http://ws.service.creditease.com/MessageService2.0/batchOrderMsgSendJaxbRequest"
	si.RequestList=p
	sr, err := webservice.CallService(si, s.Url)
	if err != nil {
		return nil, err
...
func (s *MessageServiceJaxbImplService) BatchCustomMessageSendJaxb(p MessageReqJaxb) (r *MessageResJaxb, err error) {
	...
	si.Action = "http://ws.service.creditease.com/MessageService2.0/batchCustomMessageSendJaxbRequest"
	si.RequestList=p
	sr, err := webservice.CallService(si, s.Url)
	if err != nil {
		return nil, err
...
func (s *MessageServiceJaxbImplService) QueryModel(p string) (r *SmsMessageModel, err error) {
	si := QueryModel{}
	si.Action = "http://ws.service.creditease.com/MessageService2.0/queryModelRequest"
	si.RequestList=p
	sr, err := webservice.CallService(si, s.Url)
	if err != nil {
		return nil, err
